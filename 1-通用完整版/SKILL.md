...
2. 若工作区缺 `design-assets/`，先从 Skill 复制过去（保证背景图+毛笔字体不丢）：
   ```bash
...
3. 用户导出的 HTML 同样依赖 `design-assets/`（背景图+字体），提示用户把 Skill 的 `design-assets/` 放到导出文件同目录。
...
## 7. 故障排查（FAQ）
- **海报背景空白 / 无毛笔字**：工作区缺 `design-assets/` → 按 §4.2 第 2 步复制。
...
- **资源层**：工作区缺 `design-assets/` 时按 §4.2 第 2 步自动复制，保证背景图/毛笔字体不丢。
